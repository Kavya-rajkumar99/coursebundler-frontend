    ...
    return(
        <Stack direction={["column","row"]} padding={"8"} spacing={["4","16"]}>
            ...
            <VStack justifyContent={"center"} alignItems={["center","flex-start"]}>
                <Heading textAlign={["center","left"]} size={["md","xl"]}>Kavya R</Heading>
                <Text textAlign={["center","left"]}>Hi, we are passionate full-stack developers. Our mission is to provide quality content at a reasonable price.</Text>
            </VStack>
...
const TermsAndConditions = ({termsAndConditions}) => {
    return(
        <Box textAlign={["center","left"]}>
            <Heading size="md" my="4">Terms &amp; Conditions</Heading>
            <Box h="sm" p="4" overflowY={"scroll"}>
                <Text  fontFamily="heading" letterSpacing={"widest"}>{termsAndConditions}</Text>
                <Heading size="xs" my="4" >Refund is only applicable for cancellation within 7 days</Heading>
            </Box>
        </Box>
    )
}
const About = () => {
  return (
    <Container maxW={"container.lg"} padding="16" boxShadow={"lg"}>
     ...
     <HStack my="4" p="4">
        <RiSecurePaymentFill />
        <Heading textTransform={"uppercase"} fontFamily = "sans-serif" size={"xs"}>Payment is secured by Razorpay</Heading>
     </HStack>
    </Container>
  )
}
